...
final class TeamClaudeTableView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for (i, row) in health.accounts.enumerated() {
            ...
            let displayPair = teamClaudeQuotaPair(for: row)
            let wkColor = tone(displayPair?.weeklyPercent)
            drawText(displayPair.map { percentText($0.weeklyPercent) } ?? "동기화중", innerX + 450, y + 5, rowFont, displayPair != nil ? wkColor : yellow)
            bar(displayPair?.weeklyPercent, x: innerX + 494, y: y + 10, width: 74, color: wkColor)
            drawText(formatTeamClaudeDuration(displayPair?.weeklyResetSeconds), innerX + 578, y + 5, smallFont, muted)

            let fbColor = tone(displayPair?.fablePercent)
            drawText(displayPair.map { percentText($0.fablePercent) } ?? "동기화중", innerX + 620, y + 5, rowFont, displayPair != nil ? fbColor : yellow)
            bar(displayPair?.fablePercent, x: innerX + 682, y: y + 10, width: 48, color: fbColor)
            drawText(formatTeamClaudeDuration(displayPair?.fableResetSeconds), innerX + 738, y + 5, smallFont, muted)
...
final class StatusMenuDashboardView: NSView {
    ...
    static func teamContentHeight(_ health: TeamClaudeHealth?) -> CGFloat {
        ...
        return 238 + CGFloat(health.accounts.count) * 28
    }
    ...
    static func codexHeight(_ health: CodexHealth?) -> CGFloat {
        health == nil ? 0 : CodexStatusView.preferredHeight
    }

    static func preferredHeight(teamClaude: TeamClaudeHealth?, codex: CodexHealth?, usage: UsageData?) -> CGFloat {
        let team = teamHeight(teamClaude)
        let codex = codexHeight(codex)
    ...
    func configure(
        ...
        if let codex = codex {
            let view = CodexStatusView(frame: NSRect(x: 0, y: y, width: bounds.width, height: CodexStatusView.preferredHeight))
            view.health = codex
            view.usage = usage
            addSubview(view)
            codexView = view
            y += CodexStatusView.preferredHeight + 4
        }
    ...
    func updateContent(
        ...
        if structureChanged {
            frame.size.height = Self.preferredHeight(teamClaude: teamClaude, codex: codex, usage: usage)
            configure(
...
/// The launchd label under which the TeamClaude proxy daemon is registered, if any.
/// Set `TEAMCLAUDE_LAUNCHD_LABEL` (e.g. `com.example.teamclaude`) to enable a fast
/// `launchctl kickstart` restart; when unset the app falls back to `teamclaude restart`.
func teamClaudeLaunchdLabel() -> String? {
    guard let label = ProcessInfo.processInfo.environment["TEAMCLAUDE_LAUNCHD_LABEL"],
...
func kickstartTeamClaudeServer() -> Bool {
    guard let label = teamClaudeLaunchdLabel() else { return false }
    let task = Process()
...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    var isRefreshingStatus: Bool { isRefreshingTeamClaude || isRefreshingCodex }
    ...
    func refreshOpenDashboard() {
        guard let dashboard = openDashboardView else { return }
    ...
    func loadCodexStatusInBackground() {
        ...
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            DispatchQueue.main.async {
                ...
                self.refreshOpenDashboard()
                self.updateTitle()
                print("CODEX-REFRESH: status=\(codex.statusLabel) calls=\(codex.todayCalls)/\(codex.weekCalls)")
                fflush(stdout)
            }
        }
    }
    ...
    func menuWillOpen(_ menu: NSMenu) {
        ...
        let dashboardContentHeight = StatusMenuDashboardView.preferredHeight(teamClaude: currentTeamClaude, codex: currentCodex, usage: currentData)
        let screenHeight = statusItem?.button?.window?.screen?.visibleFrame.height
            ?? NSScreen.main?.visibleFrame.height
            ?? 900
        let actionAreaHeight = CGFloat(7) * MenuActionRowView.preferredHeight + 72
        ...
        let refreshDetail = isFetchingUsageQuick ? "오늘 사용량 갱신 중" : (isFetching ? "전체 사용량 갱신 중" : (isRefreshingStatus ? "상태 갱신 중" : "즉시 갱신"))
        ...
        let measurementUnavailableCount = currentTeamClaude?.measurementUnavailableCount ?? 0
        let quotaLimitedCount = currentTeamClaude?.quotaLimitedCount ?? 0
        let measureDetail = isMeasuringTeamClaude
            ? (teamClaudeMeasureDetail ?? "진행 중")
            : (teamClaudeMeasureDetail
                ?? (unmeasuredCount > 0
                    ? "\(unmeasuredCount)개 지금 측정 가능"
                    : (measurementUnavailableCount > 0
                        ? "\(measurementUnavailableCount)개 상태 확인 필요"
                        : (quotaLimitedCount > 0 ? "측정 완료 · \(quotaLimitedCount)개 라우팅 한도 대기" : "모든 계정 측정 완료"))))
    ...
    func openTeamClaudeTerminal(commandTitle: String, arguments: [String]) {
        let exe = resolveTeamClaudeExecutable()
        let quotedExe = shellQuote(exe)
        let argString = arguments.map(shellQuote).joined(separator: " ")
        // Prefer a fast launchd kickstart when TEAMCLAUDE_LAUNCHD_LABEL is set,
        // otherwise restart via the teamclaude CLI (portable default).
        let restart: String
        if let label = teamClaudeLaunchdLabel() {
            restart = "launchctl kickstart -k gui/$(id -u)/\(shellQuote(label)) || \(quotedExe) restart"
        } else {
            restart = "\(quotedExe) restart"
        }
        let command = [
            "clear",
            "echo \(shellQuote("TeamClaude: \(commandTitle)"))",
            "\(quotedExe) \(argString)",
            "status=$?",
