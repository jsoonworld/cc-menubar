...
func teamClaudeMeasurementIssue(
    ...
    let knownMaximum = [sessionPercent, weeklyPercent, fablePercent].compactMap { $0 }.max()
...
func teamClaudeAccountIsUsable(
    ...
    guard enabled,
          ...
          let weeklyPercent = weeklyPercent,
          let fablePercent = fablePercent else {
        return false
    }
    return max(sessionPercent, weeklyPercent, fablePercent) < thresholdPercent
}
